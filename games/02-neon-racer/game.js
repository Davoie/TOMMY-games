...
class Game {
  constructor(canvas) {
    ...
    // ---- 驾驶舱视角参数 ----
    this.HORIZON_Y=Math.floor(this.H*0.42);  // 地平线 (坐姿比站立低)
    this.FOCUS=250;                           // 焦距 (越小透视越强)
    this.ROAD_W=0.55;                         // 世界道路半宽
    ...
    // ---- 驾驶 ----
    this.speed=0;this.maxSpeed=120;
    this.accel=2.5;this.brakeForce=6;this.friction=0.4;
    this.displaySpeed=0;

    // ---- 物理 ----
    this.player={x:0,steer:0,yaw:0,skid:false,skidT:0};
    this.steerSpeed=0.06;this.steerReturn=0.08;this.yawReturn=0.12;
    this.skidThresh=80;
    ...
    this.FAR_D=2000;this.NEAR_D=12;

    // ---- 装饰 ----
    this.stars=[];this._iStars();
    this.buildings=[];this._iBldg();
    this.lampposts=[];this.billboards=[];
    this.particles=[];this.smoke=[];
  ...
  _buildStrips() {
    const n=300;
    for(let i=0;i<=n;i++){
      const sy=this.HORIZON_Y+i*(this.H-this.HORIZON_Y)/n;
      const d=sy>this.HORIZON_Y?(this.H-this.HORIZON_Y)*this.FOCUS/(sy-this.HORIZON_Y)-this.FOCUS:999999;
      const sc=this.FOCUS/(d+this.FOCUS);
      this.strips.push({sy,d,sc});
    }
    this.FAR_D=this.strips[Math.floor(n*0.03)].d;
  }

  _proj(d) {
    if(d<=0)return{sy:this.H,sc:1.2};
    const sc=this.FOCUS/(d+this.FOCUS);
    return{sy:this.HORIZON_Y+(this.H-this.HORIZON_Y)*sc,sc:Math.min(sc,1.25)};
  }

  _roadX(d) {
    // 弯道 + 玩家横向偏移 (路面中心相对于视野中心)
    return this.curve*d*d*0.0004 - this.player.x*350*d*0.0008;
  }

  // ===== 初始化 =====
  _iStars(){for(let l=0;l<3;l++)for(let i=0;i<40;i++)this.stars.push({x:Math.random()*this.W,y:Math.random()*this.HORIZON_Y,r:Math.random()*(l===0?1.2:l===1?1.8:2.5),tw:Math.random()*Math.PI*2,sp:0.2+l*0.3,ly:l,op:0.4+l*0.3});}
  _iBldg(){for(let i=0;i<30;i++)this.buildings.push({x:Math.random()*this.W*1.3-this.W*0.15,w:Math.random()*40+15,h:Math.random()*80+40,ly:0,ws:Array.from({length:Math.floor(Math.random()*6)},()=>({x:Math.random()*0.7+0.1,y:Math.random()*0.7+0.1,on:Math.random()>0.4})),neon:Math.random()>0.7?['#ff00ff','#00ffff','#ff4488','#ffaa00'][Math.floor(Math.random()*4)]:null});for(let i=0;i<20;i++)this.buildings.push({x:Math.random()*this.W*1.2-this.W*0.1,w:Math.random()*70+30,h:Math.random()*120+60,ly:1,ws:Array.from({length:Math.floor(Math.random()*10)},()=>({x:Math.random()*0.7+0.1,y:Math.random()*0.7+0.1,on:Math.random()>0.35})),neon:Math.random()>0.5?['#ff00ff','#00ffff','#ff4488','#ffaa00'][Math.floor(Math.random()*4)]:null});}

  // ===== 事件 =====
  _bind(){
    window.addEventListener('keydown',e=>{if(!e.key.startsWith('Arrow'))return;e.preventDefault();this.keys[e.key]=true;if(!this.started&&!this.gameOver){this.start();return;}});
    window.addEventListener('keyup',e=>{if(!e.key.startsWith('Arrow'))return;e.preventDefault();this.keys[e.key]=false;});
    this.cv.addEventListener('touchstart',e=>{e.preventDefault();SE.init();if(!this.started&&!this.gameOver){this.start();return;}if(this.gameOver)return;const x=(e.touches[0].clientX-this.cv.getBoundingClientRect().left)/this.cv.getBoundingClientRect().width;if(x<0.4)this.keys.ArrowLeft=true;else if(x>0.6)this.keys.ArrowRight=true;else this.keys.ArrowUp=true;},{passive:false});
    this.cv.addEventListener('touchend',e=>{e.preventDefault();this.keys={};},{passive:false});
    this.cv.addEventListener('touchmove',e=>{e.preventDefault();this.keys={};const x=(e.touches[0].clientX-this.cv.getBoundingClientRect().left)/this.cv.getBoundingClientRect().width;if(x<0.4)this.keys.ArrowLeft=true;else if(x>0.6)this.keys.ArrowRight=true;else this.keys.ArrowUp=true;},{passive:false});
  }

  // ===== 开始/结束 =====
  start(){this.started=true;this.gameOver=false;this.score=0;this.speed=80;this.frame=0;this.lives=3;this.invincible=0;const p=this.player;p.x=0;p.steer=0;p.yaw=0;p.skid=false;p.skidT=0;this.curve=0;this.targetCurve=0;this.curveTimer=0;this.obstacles=[];this.spawnTimer=0;this.particles=[];this.smoke=[];this.billboards=[];this.lampposts=[];this.displaySpeed=80;document.getElementById('overlayStart').classList.add('hidden');document.getElementById('overlayEnd').classList.add('hidden');BGM.start();this._loop();}
  ...
  // 更新
  // ============================================================
  _update(){this._uInput();this._uSpeed();this._uSteer();this._uPhys();this._uCurve();this._uObst();this._uPart();this._uDecor();}

  _uInput(){const k=this.keys;let ts=0;if(k.ArrowLeft)ts=-1;if(k.ArrowRight)ts=1;if(k.ArrowLeft&&k.ArrowRight)ts=0;this.player.targetSteer=ts;}
  ...
  _col(o){if(o.d<this.NEAR_D||o.d>70)return false;const pj=this._proj(o.d);const rx=this._roadX(o.d);const rw=this.ROAD_W*pj.sc*this.W;const ox=this.W/2+rx+o.lat*rw*0.8;const ow=(o.type==='truck'?40:o.type==='racer'?22:28)*pj.sc*1.1;const oh=(o.type==='truck'?60:o.type==='racer'?32:45)*pj.sc*1.1;const pw=36,ph=60;const px=this.W/2-pw/2;const py=this.H-ph+15;const pad=4;return px+pad<ox+ow/2-pad&&px+pw-pad>ox-ow/2+pad&&py+pad<pj.sy+oh/2-pad&&py+ph-pad>pj.sy-oh/2+pad;}
  ...
  // 绘制 — 真第一人称
  // ============================================================
  _draw(){const ctx=this.ctx,W=this.W,H=this.H;ctx.clearRect(0,0,W,H);this._dSky(ctx);this._dCity(ctx);this._dRoad(ctx);this._dDecor(ctx);this._dObst(ctx);this._dCockpit(ctx);this._dParts(ctx);this._dHUD(ctx);}

  // ---- 天空 ----
  _dSky(ctx){
    const g=ctx.createLinearGradient(0,0,0,this.HORIZON_Y+20);
    g.addColorStop(0,'#0a0a2e');g.addColorStop(0.5,'#1a0a3e');g.addColorStop(1,'#2a1040');
    ctx.fillStyle=g;ctx.fillRect(0,0,this.W,this.HORIZON_Y+20);
    ctx.fillStyle='#ffddaa';ctx.shadowColor='rgba(255,200,150,0.4)';ctx.shadowBlur=40;
    ctx.beginPath();ctx.arc(this.W-100,55,30,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;
  ...
  // ---- 城市 ----
  _dCity(ctx){
    const fg=ctx.createLinearGradient(0,this.HORIZON_Y-30,0,this.HORIZON_Y+60);
    fg.addColorStop(0,'rgba(10,10,46,0)');fg.addColorStop(0.5,'rgba(20,10,50,0.3)');fg.addColorStop(1,'rgba(20,10,50,0.85)');
    ctx.fillStyle=fg;ctx.fillRect(0,this.HORIZON_Y-30,this.W,90);
  ...
  _dRoad(ctx){
    ...
    for(let i=0;i<ss.length-1;i++){
      const s1=ss[i],s2=ss[i+1];
      if(s2.sy<this.HORIZON_Y-5)continue;
      ...
      // 路面梯形
      const brt=0.18+s1.sc*0.28;
      ctx.fillStyle=`rgb(${Math.floor(36*brt)},${Math.floor(36*brt)},${Math.floor(56*brt)})`;
      ...
      const rd=(Math.floor(s1.d/8)%2===0);
      ...
      if(Math.floor(s1.d/10)%2===0){
        ctx.strokeStyle=`rgba(255,255,255,${0.3+s1.sc*0.35})`;
        ctx.lineWidth=Math.max(1.5,2.5*s1.sc);
        ctx.beginPath();ctx.moveTo(W/2+rx1,s1.sy);ctx.lineTo(W/2+rx2,s2.sy);ctx.stroke();
      }

      // 霓虹灯带
      if(i%8===0){
        ctx.shadowColor='#ff00ff';ctx.shadowBlur=10*s1.sc;
        ctx.strokeStyle='#ff00ff';ctx.lineWidth=2.5*s1.sc;
        ctx.beginPath();ctx.moveTo(tl,s1.sy);ctx.lineTo(bl,s2.sy);ctx.stroke();
        ctx.shadowColor='#00ffff';ctx.strokeStyle='#00ffff';
        ctx.beginPath();ctx.moveTo(tr,s1.sy);ctx.lineTo(br,s2.sy);ctx.stroke();
        ctx.shadowBlur=0;
      }
    }
  }
  ...
  _dObst(ctx){
    ...
    for(const o of sorted){
      if(o.d<=0||o.d>this.FAR_D+400)continue;
      const pj=this._proj(o.d);
      if(pj.sy<this.HORIZON_Y||pj.sy>this.H+20)continue;
      const rx=this._roadX(o.d);
      const rw=this.ROAD_W*pj.sc*W;
      const ox=W/2+rx+o.lat*rw*0.78;
      ...
      // 前灯 (它们朝向我们)
      ctx.fillStyle='#ffff88';ctx.shadowColor='#ffff88';ctx.shadowBlur=3*pj.sc;
      ctx.fillRect(ox-bw*0.32,pj.sy-bh-2*pj.sc,bw*0.15,3*pj.sc);
      ctx.fillRect(ox+bw*0.18,pj.sy-bh-2*pj.sc,bw*0.15,3*pj.sc);ctx.shadowBlur=0;
      // 逆行箭头
  ...
  _dCockpit(ctx){
    ...
    // 抖动
    const shake=(this.speed>0?this.speed*0.015:0)+(p.skid?2.5:0);
    ...
    ctx.fillRect(0,dashY,W,130);
    // 仪表台渐变
    const dashG=ctx.createLinearGradient(0,dashY,0,dashY+30);
    dashG.addColorStop(0,'#141428');dashG.addColorStop(0.3,'#1a1a30');dashG.addColorStop(1,'#0d0d1a');
    ctx.fillStyle=dashG;ctx.fillRect(0,dashY,W,30);

    // ---- 仪表台缝线 ----
    ctx.strokeStyle='rgba(255,0,255,0.15)';ctx.lineWidth=1;ctx.setLineDash([3,8]);
    ctx.beginPath();ctx.moveTo(W*0.2,dashY+15);ctx.lineTo(W*0.8,dashY+15);ctx.stroke();ctx.setLineDash([]);

    // ===== 双炮筒仪表 (速度表 + 转速表) =====
    const gaugeCY=dashY+45,gaugeR=28;
    // 左: 转速表
    this._drawGauge(ctx,W*0.28,gaugeCY,gaugeR,this.speed/this.maxSpeed,'x1000','RPM','#00ffff','#0088aa');
    // 右: 速度表
    this._drawGauge(ctx,W*0.72,gaugeCY,gaugeR,this.displaySpeed/300,'KM/H','SPEED','#ff00ff','#8800aa');

    // ===== 方向盘底座 (转向柱) =====
    const wheelCX=W/2+sx;
    const wheelCY=H-60+sy;
    ctx.fillStyle='#111';
    ctx.beginPath();ctx.moveTo(wheelCX-8,wheelCY-25);ctx.lineTo(wheelCX+8,wheelCY-25);
    ctx.lineTo(wheelCX+5,dashY+10);ctx.lineTo(wheelCX-5,dashY+10);ctx.closePath();ctx.fill();
    ctx.strokeStyle='#333';ctx.lineWidth=2;
    ctx.beginPath();ctx.moveTo(wheelCX,wheelCY-25);ctx.lineTo(wheelCX,dashY+10);ctx.stroke();

    // ===== 方向盘 =====
    ctx.save();ctx.translate(wheelCX,wheelCY);
    const steerVis=p.steer*Math.PI*0.35; // ← → 最大约 ±63°
    ctx.rotate(steerVis);

    // 方向盘外环光晕
    ctx.shadowColor='rgba(80,80,80,0.4)';ctx.shadowBlur=8;

    // 外环
    const wrX=58,wrY=20; // 椭圆半径
    ctx.fillStyle='#1a1a1a';
    ctx.beginPath();ctx.ellipse(0,0,wrX,wrY,0,0,Math.PI*2);ctx.fill();
    ...
    // 左辐条
    ctx.beginPath();ctx.moveTo(-15,5);ctx.lineTo(-wrX+10,wrY-2);ctx.stroke();
    // 右辐条
    ctx.beginPath();ctx.moveTo(15,5);ctx.lineTo(wrX-10,wrY-2);ctx.stroke();
    // 下辐条
    ctx.beginPath();ctx.moveTo(0,12);ctx.lineTo(0,wrY-2);ctx.stroke();
    ...
    ctx.beginPath();ctx.arc(0,-wrY+3,4,0,Math.PI*2);ctx.fill();

    ctx.shadowBlur=0;
    ctx.restore();

    // ===== 双手 =====
    ctx.save();ctx.translate(wheelCX,wheelCY);ctx.rotate(steerVis);
    // 左手 (9点钟)
    ctx.fillStyle='#d4a574';ctx.strokeStyle='#b8956a';ctx.lineWidth=2;
    ctx.beginPath();
    ctx.arc(-wrX+3,3,9,0,Math.PI*2);ctx.fill();ctx.stroke();
    // 右手 (3点钟)
    ctx.beginPath();
    ctx.arc(wrX-3,3,9,0,Math.PI*2);ctx.fill();ctx.stroke();
    // 手指缝
    for(let s=-1;s<=1;s+=2){
      ...
      for(let f=-1;f<=1;f++){
        ctx.beginPath();ctx.moveTo(s*(wrX-3)+f*3,6-f*2);
        ctx.lineTo(s*(wrX-3)+f*3,14-f);ctx.stroke();
      }
    }
    ctx.restore();
    ...
    ctx.save();ctx.translate(wheelCX,wheelCY);ctx.rotate(steerVis);
    // 左拨片
    ctx.fillStyle='#555';ctx.beginPath();ctx.roundRect(-wrX-8,-4,10,16,3);ctx.fill();
    ctx.fillStyle='#f00';ctx.font='bold 6px monospace';ctx.textAlign='center';
    ctx.fillText('-',-wrX-3,6);
    // 右拨片
    ctx.fillStyle='#555';ctx.beginPath();ctx.roundRect(wrX-2,-4,10,16,3);ctx.fill();
    ctx.fillStyle='#0f0';ctx.fillText('+',wrX+3,6);
    ctx.restore();
    ...
    ctx.beginPath();
    ctx.moveTo(W*0.26,hoodTop-10);
    ctx.quadraticCurveTo(W*0.3,H-30,W*0.33,H+8);
    ctx.lineTo(W*0.67,H+8);
    ctx.quadraticCurveTo(W*0.7,H-30,W*0.74,hoodTop-10);
    ctx.closePath();ctx.fill();

    // 引擎盖光泽
    const hg=ctx.createLinearGradient(W/2,hoodTop-10,W/2,H);
    hg.addColorStop(0,'rgba(25,25,40,0.5)');
    hg.addColorStop(0.5,'rgba(35,35,50,0.2)');
    hg.addColorStop(1,'rgba(10,10,20,0.8)');
    ctx.fillStyle=hg;
    ctx.fillRect(W*0.28,hoodTop-5,W*0.44,H-hoodTop+20);

    // 引擎盖中线隆起
    ctx.strokeStyle='rgba(255,255,255,0.04)';ctx.lineWidth=2;
    ctx.beginPath();ctx.moveTo(W/2+sx*0.5,hoodTop-5);ctx.lineTo(W/2,H-10);ctx.stroke();

    // 引擎盖两侧反光
    ctx.fillStyle='rgba(255,255,255,0.02)';
    ctx.fillRect(W*0.28,hoodTop-5,W*0.06,H-hoodTop+20);
    ctx.fillRect(W*0.66,hoodTop-5,W*0.06,H-hoodTop+20);

    // ===== A柱 (前挡风边框) =====
    // 左
    const ag=ctx.createLinearGradient(0,0,W*0.3,0);
    ag.addColorStop(0,'#0a0a16');ag.addColorStop(1,'#151525');
    ctx.fillStyle=ag;
    ctx.beginPath();
    ctx.moveTo(0,this.HORIZON_Y-40);
    ctx.lineTo(W*0.07,this.HORIZON_Y-25);
    ctx.lineTo(W*0.22,hoodTop-15);
    ctx.lineTo(W*0.08,hoodTop-10);
    ctx.lineTo(0,hoodTop+5);
    ctx.closePath();ctx.fill();
    // 右
    const ag2=ctx.createLinearGradient(W,0,W*0.7,0);
    ag2.addColorStop(0,'#0a0a16');ag2.addColorStop(1,'#151525');
    ctx.fillStyle=ag2;
    ctx.beginPath();
    ctx.moveTo(W,this.HORIZON_Y-40);
    ctx.lineTo(W*0.93,this.HORIZON_Y-25);
    ctx.lineTo(W*0.78,hoodTop-15);
    ctx.lineTo(W*0.92,hoodTop-10);
    ctx.lineTo(W,hoodTop+5);
    ctx.closePath();ctx.fill();

    // A柱内缝
    ctx.strokeStyle='rgba(255,255,255,0.06)';ctx.lineWidth=1;
    ctx.beginPath();ctx.moveTo(W*0.08,this.HORIZON_Y-32);ctx.lineTo(W*0.22,hoodTop-12);ctx.stroke();
    ctx.beginPath();ctx.moveTo(W*0.92,this.HORIZON_Y-32);ctx.lineTo(W*0.78,hoodTop-12);ctx.stroke();

    // ===== 顶部遮阳带 + 后视镜 =====
    ctx.fillStyle='rgba(5,5,15,0.55)';
    ctx.fillRect(0,this.HORIZON_Y-40,W,22);

    // 内后视镜
    ctx.fillStyle='#1a1a28';ctx.strokeStyle='#444';ctx.lineWidth=2.5;
    ctx.beginPath();ctx.roundRect(W/2-22,this.HORIZON_Y-38,44,18,10);ctx.fill();ctx.stroke();
    ctx.fillStyle='rgba(15,25,50,0.7)';
    ctx.beginPath();ctx.roundRect(W/2-21,this.HORIZON_Y-37,42,16,9);ctx.fill();
    // 镜子高光
    ctx.fillStyle='rgba(255,255,255,0.06)';
    ctx.beginPath();ctx.roundRect(W/2-19,this.HORIZON_Y-36,16,6,3);ctx.fill();
    // 镜子支架
    ctx.fillStyle='#333';ctx.fillRect(W/2-2,this.HORIZON_Y-42,4,6);
  }

  // ---- 仪表盘辅助: 单炮筒 ----
  _drawGauge(ctx,cx,cy,r,val,label,subLabel,accent,dim){
    const pct=Math.min(1,Math.max(0,val));
    // 底色
    ctx.fillStyle='#0a0a16';ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='#333';ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.stroke();
    // 刻度弧
    const startA=Math.PI*0.75,endA=Math.PI*2.25;
    ctx.strokeStyle='#2a2a3e';ctx.lineWidth=r*0.3;ctx.lineCap='round';
    ctx.beginPath();ctx.arc(cx,cy,r*0.65,startA,endA);ctx.stroke();
    // 进度弧
    const sweep=pct*(endA-startA);
    ctx.strokeStyle=accent;ctx.shadowColor=accent;ctx.shadowBlur=6;
    ctx.beginPath();ctx.arc(cx,cy,r*0.65,startA,startA+sweep);ctx.stroke();ctx.shadowBlur=0;
    // 数值
    ctx.fillStyle='#fff';ctx.font=`bold ${Math.floor(r*0.45)}px monospace`;ctx.textAlign='center';
    ctx.fillText(String(Math.round(val*100)),cx,cy+r*0.2);
    // 标签
    ctx.fillStyle=dim;ctx.font=`${Math.floor(r*0.25)}px monospace`;
    ctx.fillText(subLabel,cx,cy+r*0.55);
    ctx.fillStyle=accent;ctx.font=`bold ${Math.floor(r*0.22)}px monospace`;
    ctx.fillText(label,cx,cy-r*0.7);
    // 指针
    ctx.save();ctx.translate(cx,cy);
    const a=startA+sweep;ctx.rotate(a);
    ctx.strokeStyle=accent;ctx.lineWidth=1.5;
    ctx.beginPath();ctx.moveTo(-r*0.15,0);ctx.lineTo(r*0.55,0);ctx.stroke();
  ...
  _dParts(ctx){
    ...
    for(const pp of this.smoke){ctx.fillStyle=`rgba(180,180,200,${pp.life/25*0.35})`;ctx.beginPath();ctx.arc(pp.x,pp.y,pp.r,0,Math.PI*2);ctx.fill();}
    ctx.globalAlpha=1;
  }

  // ---- HUD (精简: 信息已在仪表盘展示) ----
  _dHUD(ctx){
    const W=this.W,H=this.H,spd=Math.round(this.displaySpeed),p=this.player;

    // 生命 (左上)
    ctx.textAlign='left';ctx.font='18px sans-serif';
    ctx.fillText('❤️'.repeat(this.lives)+'🖤'.repeat(3-this.lives),10,this.HORIZON_Y-18);

    // 分数 (右上)
    ctx.textAlign='right';ctx.font='bold 14px monospace';ctx.fillStyle='#ff00ff';
    ctx.shadowColor='#ff00ff';ctx.shadowBlur=6;
    ctx.fillText('🏆 '+String(this.score),W-10,this.HORIZON_Y-18);ctx.shadowBlur=0;

    // 打滑
    if(p.skid){ctx.textAlign='center';ctx.font='bold 14px sans-serif';ctx.fillStyle='#ff4488';ctx.shadowColor='#ff4488';ctx.shadowBlur=10;ctx.fillText('⚠ 打滑漂移！',W/2,H-135);ctx.shadowBlur=0;}

    // 操作提示
    ctx.textAlign='center';ctx.font='10px sans-serif';ctx.fillStyle='rgba(255,255,255,0.2)';
    ctx.fillText('↑油门  ↓刹车  ←→转向',W/2,H-6);

    // 速度线
    if(this.speed>180){
      ctx.strokeStyle='rgba(255,255,255,0.12)';
      for(let i=0;i<6;i++){ctx.lineWidth=1+Math.random()*1.5;ctx.beginPath();const lx=W*0.2+Math.random()*W*0.6;ctx.moveTo(lx,this.HORIZON_Y);ctx.lineTo(lx+(Math.random()-0.5)*30,this.HORIZON_Y+Math.random()*50);ctx.stroke();}
    }
  }
}
